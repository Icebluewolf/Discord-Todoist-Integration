...
class AddDesc(discord.ui.Modal):
    def __init__(self, task: Task, view: AddTaskOptions):
        ...
        self.add_item(
            discord.ui.InputText(
                label="Enter Description", required=False, value=task.description
            )
        )
        self.add_item(
            discord.ui.InputText(
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        await interaction.followup.edit_message(
            self.parent_view.message.id, embed=await get_task_info(self.task)
        )
...
async def todo(
    ...
    try:
        ...
        await ctx.respond(
            embed=await get_task_info(response), view=view, ephemeral=True
        )
...
async def view_task(
    ...
    await ctx.respond(
        embed=await get_task_info(response),
        view=AddTaskOptions(response),
        ephemeral=True,
    )
